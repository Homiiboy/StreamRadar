...
const serviceNames = [...tmdb.SERVICE_DEFINITIONS.map(s => s.name), 'FX', 'Hulu', 'Peacock', 'AMC+', 'BBC'];
...
function accentFor(service){ return ({'Netflix':'#ff3158','Disney+':'#2a7cff','Prime Video':'#39a8ff','HBO Max':'#7c6dff','Apple TV+':'#d9e2ef','Paramount+':'#4386ff','Crunchyroll':'#ff8c31','Sky / WOW':'#ff5dcc','Joyn':'#f4d44d','RTL+':'#38d59f','ORF':'#e84855','FX':'#ff7a59','Hulu':'#55e58a','Peacock':'#8f7aff','AMC+':'#ffb45a','BBC':'#e6e6e6'})[service]||'#62f7c7'; }
...
function renderServices(){ $('#serviceStrip').innerHTML=serviceNames.map(name=>{ const p=providerFor(name); const logo=p?.logoPath?`<img src="${tmdb.image(p.logoPath,'w92')}" alt="" loading="lazy"/>`:'<span class="service-dot"></span>'; const sourceOnly=state.mode==='live'&&(!p||!p.available); return `<button class="service-chip ${state.service===name?'active':''} ${sourceOnly?'source-only':''}" data-service="${escapeHTML(name)}" title="${sourceOnly?'Original-Marke/Quelle; aktuell kein direkter AT-Provider gemappt':`${name} filtern`}">${logo}${escapeHTML(name)}</button>`; }).join(''); $$('.service-chip').forEach(b=>b.onclick=()=>{state.service=state.service===b.dataset.service?'all':b.dataset.service;renderServices();renderReleases();}); }
...
function cardTemplate(item){
  ...
  const rightBadge=item.originalBrand?`<span class="badge original-brand" title="Original-Erkennung: ${escapeHTML(item.originalConfidence||'')}">${escapeHTML(item.originalBrand)} ORIGINAL</span>`:(state.mode==='demo'?'<span class="badge demo">DEMO</span>':'');
...
async function enrichOriginals(token){
  ...
  try{ await tmdb.enrichOriginalMetadata(candidates,token,(done,total)=>{ setDataStatus('live','TMDB live',`Original-Marken werden erkannt ${done}/${total} …`); if(done%8===0||done===total)renderReleases(); },72); }
  finally{ state.enriching=false; setDataStatus('live','TMDB + TVmaze',`${state.releases.length} reale Titel · Originals erkannt · Region Österreich.`); renderReleases(); }
}
...
async function openDetails(id){
  ...
  try{ if(!details){details=await tmdb.getDetails(item.mediaType,item.tmdbId,localStorage.getItem(TOKEN_KEY));state.detailCache.set(key,details); if(details.inferredOriginalBrand&&!item.originalBrand){item.original=true;item.originalBrand=details.inferredOriginalBrand;item.originalConfidence=details.originalConfidence;renderReleases();}} renderDetail(item,details,null,item.mediaType==='tv');
...
function renderDetail(item,details,maze,loading){
  ...
  const brand=item.originalBrand||details?.inferredOriginalBrand; const confidence=item.originalConfidence||details?.originalConfidence;
