...
(() => {
  ...
  function loadConfig() {
    ...
    if (!['catalog-home','catalog-movies','catalog-series','catalog-anime','catalog-watchlist','discover','calendar','upcoming','watchlist'].includes(merged.defaultView)) merged.defaultView = 'catalog-home';
    ...
    return `<div class="settings-center">
      ...
      <div class="settings-main">
        <section class="settings-page" data-settings-page="general"><div class="settings-page-head"><span class="section-kicker">ALLGEMEIN</span><h2>StreamRadar für dich</h2><p>Lege fest, wie die Desktop-App startet und wie dicht Informationen dargestellt werden.</p></div>
          ...
          <div class="setting-row"><div class="setting-copy"><strong>Standardansicht</strong><span>Wird verwendet, wenn die letzte Ansicht nicht gemerkt wird.</span></div><select class="setting-select" id="prefDefaultView"><option value="catalog-home">Entdecken</option><option value="catalog-movies">Filme</option><option value="catalog-series">Serien</option><option value="catalog-anime">Anime</option><option value="discover">Neu & aktuell</option><option value="calendar">Kalender</option><option value="upcoming">Demnächst</option><option value="catalog-watchlist">Merkliste</option></select></div>
        ...
        <section class="settings-page" data-settings-page="about"><div class="settings-page-head"><span class="section-kicker">ÜBER STREAMRADAR</span><h2>Personal Streaming Release Intelligence</h2><p>Streaming-Katalog und Release-Radar für Österreich mit TMDB, JustWatch-Providern und TVmaze-Schedule.</p></div><div class="about-version-card"><div class="about-version-mark">SR</div><div><strong>StreamRadar ${VERSION}</strong><span>Streaming Catalog & Provider Experience · Windows Desktop / Web</span></div></div><div class="settings-group"><div class="setting-row"><div class="setting-copy"><strong>Aktive UI-Dateien</strong><span>styles.css + ui.js · historische Snapshots liegen ausschließlich im Archiv.</span></div></div><div class="setting-row"><div class="setting-copy"><strong>Datenregion</strong><span>Österreich (AT) · Sprache de-DE/de-AT.</span></div></div><div class="setting-row"><div class="setting-copy"><strong>Installer</strong><span>Windows x64 MSI · für persönliche Nutzung aktuell unsigniert.</span></div></div></div></section>
      </div></div>`;
  }
  ...
  setView = function(view) {
    if (config.rememberLastView && ['catalog-home','catalog-movies','catalog-series','catalog-anime','catalog-watchlist','discover','calendar','seasons','episodes','upcoming','watchlist'].includes(view)) localStorage.setItem(LAST_VIEW_KEY, view);
  ...
  function restoreStartupView() {
    ...
    const target = ['catalog-home','catalog-movies','catalog-series','catalog-anime','catalog-watchlist','discover','calendar','seasons','episodes','upcoming','watchlist'].includes(view) ? view : config.defaultView;
